...
import { markdownManager } from '../markdown/MarkdownService';
import {
...
export type ClipboardIssueLogger = (error: unknown, context: string) => void;

export interface MarkdownClipboardOptions {
  logSchemaMismatch?: ClipboardIssueLogger;
}

const defaultClipboardIssueLogger: ClipboardIssueLogger = (error, context) => {
...
function tryNodeFromJSON(
  ...
  try {
    ...
    logSchemaMismatch(error, 'markdown clipboard: schema mismatch');
    return null;
  }
}
...
function createClipboardTextSlice(
  ...
  const logSchemaMismatch =
    options.logSchemaMismatch ?? defaultClipboardIssueLogger;
  ...
  if (isSoleDegenerateCodeBlock(json)) {
    ...
    if (normalized !== text) {
      ...
      if (retryJson && isRicherParse(json, retryJson)) {
        const retrySlice = tryNodeFromJSON(view, retryJson, logSchemaMismatch);
        if (retrySlice) return retrySlice;
      }
    }
  }

  const slice = tryNodeFromJSON(view, json, logSchemaMismatch);
...
export function createMarkdownClipboardTextParser(
  ...
  return (
    ...
    return createClipboardTextSlice(
      text,
      context,
      shouldBypassMarkdown,
      view,
      options,
    );
  };
