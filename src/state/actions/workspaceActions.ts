...
import { useWorkspaceStore, type WorkspaceFolder } from '../slices/workspaceSlice';
...
export const workspaceActions = {
  ...
  loadWorkspaceFile: async (workspacePath: string): Promise<Result> => {
    try {
      ...
      const batchResult: FolderPathResult[] = await FsService.listTreeBatch(paths);
      ...
      const rootFolders: RootFolderNode[] = batchResult
        ...
        .map((r: FolderPathResult, index: number) => ({
          ...
          displayName: config.folders[index]?.name || r.path.split('/').pop() || r.path,
          tree: r.nodes,
        }));
      
      // 原子更新所有 store
      useWorkspaceStore.getState().restoreState({
        folders: config.folders.map((f: { path: string; name?: string }, i: number) => ({
          path: f.path,
          name: f.name,
          index: i,
        })),
