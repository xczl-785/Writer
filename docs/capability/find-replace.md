...
- **entry_points**:
  ...
  - 快捷键 Cmd/Ctrl+H（替换）
  - useFindReplace hook
  - `src/core/editor/findReplace/findReplaceDomain.ts`
  - 旧入口 `src/domains/editor/domain/findReplace/findReplaceDomain.ts` re-export
- **shared_with**: none
- **check_on_change**:
  - 快捷键绑定不变
  - 查找匹配逻辑不变
...
查找替换能力提供在编辑器中查找和替换文本的功能。通过 Cmd/Ctrl+F 打开查找面板，Cmd/Ctrl+H 打开替换面板。支持遍历所有匹配项、单个替换当前匹配、全部替换所有匹配。查找匹配 domain 纯函数位于 editor-core，旧 domain 路径保留 re-export 兼容入口。
...
| collectFindTextMatches                 | 收集匹配项   | `src/core/editor/findReplace/findReplaceDomain.ts:9-36`、`src/domains/editor/domain/findReplace/findReplaceDomain.ts` re-export | 纯函数         |
...
**Evidence**: `src/core/editor/findReplace/findReplaceDomain.ts:19, 24`、`src/config/editor.ts:14`
...
**Evidence**: `src/core/editor/findReplace/findReplaceDomain.ts:18-33`
...
| findReplaceDomain   | 文本匹配逻辑正确                  | `src/core/editor/findReplace/findReplaceDomain.ts`、`src/domains/editor/domain/findReplace/findReplaceDomain.ts` re-export |
...
| findReplaceDomain    | core 文本匹配纯函数 | `src/core/editor/findReplace/findReplaceDomain.ts`      |
