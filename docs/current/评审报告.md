...
| **修正建议** | ① 创建 `spike/roundtrip/` 目录，内含最小 TipTap + `markdown-it`/`prosemirror-markdown` 脚本；② 定义 4 组断言用例（标题 H1-H6、有序/无序列表嵌套、围栏代码块含语言标识、`![alt](path)` 图片引用）；③ 在进入 Step E 前运行并把结果写入 `docs/current/SPIKE_ROUNDTRIP_REPORT.md`。预估 2-4 小时。 |
...
3. 创建 `docs/current/SPIKE_ROUNDTRIP_REPORT.md`（模板），包含：执行日期、结果（通过/未通过）、差异截图/文本
